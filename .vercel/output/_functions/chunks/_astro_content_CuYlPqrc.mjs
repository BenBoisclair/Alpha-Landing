import { o as objectType, e as dateType, n as numberType, f as arrayType, s as stringType, A as AstroError, R as RenderUndefinedEntryError, c as createComponent, u as unescapeHTML, r as renderTemplate, U as UnknownContentCollectionError, g as escape, h as renderUniqueStylesheet, i as renderScriptElement, j as createHeadAndContent, b as renderComponent } from './astro/server_CAhXBRiF.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_Bziv8O0t.mjs');
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (collection in contentCollectionToEntryMap) {
      ...
      const { default: imageAssetMap } = await import('./content-assets_CgSJKbwQ.mjs');
      const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_astro_assets_DrlSI8AY.mjs').then(n => n._);
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules_VS8K-XmR.mjs');
